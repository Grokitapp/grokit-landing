...
export default function GoogleButton({ onClick, disabled = false }: GoogleButtonProps) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="w-full flex items-center justify-center gap-2.5 px-4 py-3.5 rounded-full border border-line bg-surface-alt text-ink font-sans font-bold hover:bg-surface transition-colors disabled:opacity-40"
    >
      <GoogleIcon />
