...
export default function Onboarding() {
  ...
  const addOther = (
    ...
    setList((curr) => (curr.includes(trimmed) ? curr : [...curr, trimmed]));
    clear();
  };
  ...
  const hasOtherWorkType =
    isOtherWorkTypeSelected && otherWorkType.trim().length > 0;

  const canContinue =
    step === STEP.WORK_TYPE
      ? workTypes.length > 0 || hasOtherWorkType
      : step === STEP.TOPICS
        ? topics.length > 0
        : step === STEP.GOALS
          ? goals.length > 0
  ...
  const renderStep = () => {
    switch (step) {
      case STEP.AUTH:
        return (
          <AuthScreen
            onAuthenticated={() => setStep(STEP.WELCOME)}
          />
        );
          ...
          <WorkTypeStep
            ...
            onContinue={() => {
              const finalWorkTypes = hasOtherWorkType
                ? [...workTypes, otherWorkType.trim()]
                : workTypes;

              persist({ workTypes: finalWorkTypes });
      ...
      case STEP.T_PERFECT:
      case STEP.T_GREAT: {
        const t = STATIC_TRANSITIONS[step];
        if (!t) return null;
  ...
  /*
   * Dark onboarding canvas: question and transition
   * screens share the same visual environment.
   */
  ...
  return (
    <div
      className={`min-h-[100dvh] h-[100dvh] flex flex-col overflow-hidden ${canvasBg}`}
    >
      {/* Navigation + progress */}
      {prevStep !== undefined && (
        <div
          className={`relative z-30 w-full shrink-0 px-5 sm:px-6 pt-4 sm:pt-5 pb-1 flex items-start ${canvasBg}`}
        >
          <button
            type="button"
            onClick={() => setStep(prevStep)}
            className={`p-2 -ml-2 shrink-0 rounded-full transition-colors ${
              isDarkOnboarding
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={`flex-1 min-h-0 flex flex-col ${canvasBg}`}
        >
          {renderStep()}
        </motion.div>
      </AnimatePresence>
